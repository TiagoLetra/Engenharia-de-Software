...
class GetSolvedQuizzesTest extends SpockTest {
    ...
    def "returns solved quiz with: quizType=#quizType | conclusionDate=#conclusionDate | resultsDate=#resultsDate"() {
        ...
        def answerDetails = new MultipleChoiceAnswer(questionAnswer, [option,option2]);     //MODIFICAR APOS RESOLVER ERRO JPA
        ...
        answer.getAnswerDetails().getOptionsIds().contains(option.getId())
        answer.getAnswerDetails().getOptionsIds().contains(option2.getId())
        solvedQuizDto.getCorrectAnswers().size() == 1
        def correct = solvedQuizDto.getCorrectAnswers().get(0)
        correct.getSequence() == 0
        correct.getCorrectAnswerDetails().getCorrectOptionsIds().contains(option.getId())    //NOVO
        correct.getCorrectAnswerDetails().getCorrectOptionsIds().contains(option2.getId())    //NOVO
