...
export default class MultipleChoiceAnswer extends Vue {
  ...
  /*optionClass(index: number) {
    if (this.isReadonly) {
      if (
        !!this.correctAnswerDetails &&
        this.correctAnswerDetails.correctOptionsIds.includes(this.questionDetails.options[index].optionId)
      ) {
        return 'correct';
      } else if (
        this.answerDetails.optionsIds.includes(this.questionDetails.options[index].optionId)
      ) {
        return 'wrong';
      } else {
        return '';
      ...
      return this.answerDetails.optionsIds.includes(this.questionDetails.options[index].optionId)
        ? 'selected'
        : '';
    }
  }*/
  ...
  /*@Emit('question-answer-update')
  selectOption(optionId: number) {
    if (this.answerDetails.optionsIds.includes(optionId)) {     //NOVO
      for(var i = 0; i < this.answerDetails.optionsIds.length; i++){
        if(this.answerDetails.optionsIds[i] == optionId )this.answerDetails.optionsIds.splice(i,1);
      }
    } else {
